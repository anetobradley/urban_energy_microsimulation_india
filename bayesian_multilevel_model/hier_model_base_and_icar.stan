// iheus_neapolitan_hier_icar_rb_logit.stan
...
model {
  ...
  for(p in 1:M){
     z_p[p] ~ normal(pdee[p] + pee_1[p].*HH_size_test[p] + pee_2[p].*HH_exp_test[p] + phi_f[ward[p]], sigma_z_p);
     y_p[p] ~ normal(pay[p] + peta_1[p].*HH_size_test[p] + peta_2[p].*HH_exp_test[p] + phi_l[ward[p]], sigma_y_p);
     rb_p[p] ~ bernoulli_logit(rb_hat_test[p]+phi_rb[ward[p]]);
     v_p[p] ~ bernoulli_logit(app_hat_test[p] + phi_app[ward[p]]);
  }
...
generated quantities{
  ...
  for(m in 1:M){
    rb_test[m] = inv_logit(rb_hat_test[m]+ phi_rb[ward[m]]);
    app_test[m] = inv_logit(app_hat_test[m] + phi_app[ward[m]]);
    cook_rhat_test[m] = categorical_logit_rng((x_test[m]*beta)'); // Posterior predictive
    z_rhat_test[m] = normal_rng(d[cook_rhat_test[m]]+ c1[cook_rhat_test[m]]*HH_size_test[m] + c2[cook_rhat_test[m]]*HH_exp_test[m] + phi_f[ward[m]], sigma_f );//+ c2[cook_rhat_test[m]]*HH_exp_test[m], sigma_f);
    y_rhat_test[m] = normal_rng(a[cook_rhat_test[m]] + b1[cook_rhat_test[m]]*HH_size_test[m] + b2[cook_rhat_test[m]]*HH_exp_test[m]+ phi_l[ward[m]], sigma);
    }
