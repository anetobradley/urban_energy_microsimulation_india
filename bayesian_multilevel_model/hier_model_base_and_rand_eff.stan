...
model {
  ...
  for(p in 1:M){
     z_p[p] ~ normal(pdee[p] + pee_1[p].*HH_size_test[p] + pee_2[p].*HH_exp_test[p] + f[ward[p]], sigma_z_p);
     y_p[p] ~ normal(pay[p] + peta_1[p].*HH_size_test[p] + peta_2[p].*HH_exp_test[p] + g[ward[p]], sigma_y_p);
     rb_p[p] ~ bernoulli_logit(rb_hat_test[p] + h[ward[p]]);
     v_p[p] ~ bernoulli_logit(app_hat_test[p] + l[ward[p]]);
  }
...
generated quantities{
  ...
  for(m in 1:M){
    rb_test[m] = inv_logit(rb_hat_test[m] + h[ward[m]]);
    app_test[m] = inv_logit(app_hat_test[m] + l[ward[m]]);
    cook_rhat_test[m] = categorical_logit_rng((x_test[m]*beta)'); // Posterior predictive
    z_rhat_test[m] = normal_rng(d[cook_rhat_test[m]]+ c1[cook_rhat_test[m]]*HH_size_test[m] + c2[cook_rhat_test[m]]*HH_exp_test[m] + f[ward[m]], sigma_f );//+ c2[cook_rhat_test[m]]*HH_exp_test[m], sigma_f);
    y_rhat_test[m] = normal_rng(a[cook_rhat_test[m]] + b1[cook_rhat_test[m]]*HH_size_test[m] + b2[cook_rhat_test[m]]*HH_exp_test[m] + g[ward[m]], sigma);
    }
